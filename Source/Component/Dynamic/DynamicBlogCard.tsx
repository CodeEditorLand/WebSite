...
const DynamicBlogCard = ({ Post }: { Post: BlogPost }) => (
	<article className="StaccatoCard bg-card p-6 transition-colors hover:border-foreground">
		{Post.Tags.length > 0 && (
			<div className="mb-3 flex flex-wrap gap-2">
				{Post.Tags.map((Tag) => (
					<span
						key={Tag}
						className="bg-[color-mix(in_srgb,var(--CardForeground)_10%,transparent)] text-card-foreground opacity-70 px-2 py-0.5 font-mono text-sm font-medium uppercase tracking-widest"
					>
						{Tag}
					</span>
				))}
		...
		<h3 className="mb-2 font-mono text-sm font-medium leading-snug text-card-foreground">
			{Post.Title}
		</h3>
